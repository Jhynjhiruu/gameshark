...
parser.add_argument(
    "-w", "--which", type=str, default="loader", help="diff loader, shell or trainer"
)
...
match args.which:
    case "loader":
        ...
        basemap = f"build/loader.map"

        myimg = f"build/GS.bin"
        mymap = f"build/loader.map"
    
    case "shell":
        baseimg = f"fs/loader/shell.bin"
        basemap = f"build/shell.map"

        myimg = f"build/shell.bin"
        mymap = f"build/shell.map"
    
    case "trainer":
        baseimg = f"fs/loader/trainer.bin"
        basemap = f"build/trainer.map"

        myimg = f"build/trainer.bin"
        mymap = f"build/trainer.map"
